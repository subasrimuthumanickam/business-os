...
// TaskBoard.tsx - Modified to only show tasks for user-created projects
import React, { useState, useEffect } from 'react';
import { 
  Plus, 
  MoreVertical, 
...
const TaskBoard: React.FC = () => {
  ...
  const getUserProjects = (): string[] => {
    try {
      const savedProjects = localStorage.getItem('userProjects');
      if (savedProjects) {
        const parsed = JSON.parse(savedProjects);
        if (Array.isArray(parsed)) {
          return parsed.map((p: any) => p.projectName);
        }
      }
    } catch (error) {
  ...
  // ✅ Only keep tasks from user-created projects, remove all default tasks
  const [tasks, setTasks] = useState<Task[]>(() => {
    const stored = loadTasksFromStorage();
    const userProjects = getUserProjects();
    
    // Filter stored tasks to only include tasks from user-created projects
    const filteredTasks = stored.filter(task => userProjects.includes(task.projectName));
    
    // Return filtered tasks or empty array
  ...
  // ✅ Save tasks to localStorage - only tasks from user-created projects
  useEffect(() => {
    try {
      const userProjects = getUserProjects();
      // Only save tasks that belong to user-created projects
      const tasksToSave = tasks.filter(task => userProjects.includes(task.projectName));
      localStorage.setItem('taskBoardTasks', JSON.stringify(tasksToSave));
    } catch (error) {
      console.error('Failed to save tasks to storage:', error);
    }
  }, [tasks]);

  // ✅ When projects change, re-filter tasks
  useEffect(() => {
    const userProjects = getUserProjects();
    const stored = loadTasksFromStorage();
    const filteredTasks = stored.filter(task => userProjects.includes(task.projectName));
  ...
  const getTasksByStatus = (status: string) => {
    ...
    let filtered = tasks.filter(task => 
      task.status === status && userProjects.includes(task.projectName)
    );
    if (searchTerm) {
      filtered = filtered.filter(task => 
        task.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const getUniqueAssignees = () => {
    ...
    const assignees = tasks
      .filter(task => userProjects.includes(task.projectName))
