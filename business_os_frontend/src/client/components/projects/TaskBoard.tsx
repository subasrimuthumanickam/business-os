...
// const TaskBoard: React.FC<TaskBoardProps> = ({ projects = [] }) => {
//   const loadTasksFromStorage = (): Task[] => {
//     try {
//       const stored = localStorage.getItem('taskBoardTasks');
//       if (stored) {
...
//   // ✅ Convert project tasks to board tasks
...
//       id: `task-${project.id}-${taskName}-${Date.now()}`,
//       title: taskName,
...
//     // ✅ Merge with existing tasks (keep existing tasks that are not from projects)
//     const existingNonProjectTasks = tasks.filter(task => 
//       !projectTasks.some(pt => pt.id === task.id)
//     );
    
//     // Only update if there are project tasks
...
//     } else if (tasks.length === 0 && projectTasks.length === 0) {
//       // No tasks at all
//     }
//   }, []);
...
// export default TaskBoard;
// TaskBoard.tsx - Fixed to sync tasks when projects change
import React, { useState, useEffect } from 'react';
import { 
  Plus, 
  MoreVertical, 
...
const TaskBoard: React.FC<TaskBoardProps> = ({ projects = [] }) => {
  ...
  // ✅ Sync tasks from projects - runs when projects prop changes
  useEffect(() => {
    const syncTasks = () => {
      ...
      // ✅ Keep existing tasks that are not from projects (like bugs)
      const existingNonProjectTasks = tasks.filter(task => 
        !projectTasks.some(pt => pt.id === task.id) && 
        !task.id.startsWith('task-')
      );

      // Merge project tasks with existing non-project tasks
      const allTasks = [...projectTasks, ...existingNonProjectTasks];
    ...
    syncTasks();
  }, [projects]); // ✅ Re-run when projects prop changes
