...
// // export default TimeTracker;
// // frontend/src/client/components/projects/TimeTracker.tsx
...
//   // Timer state synced from TaskBoard via localStorage
//   const [isRunning, setIsRunning] = useState(false);
//   const [seconds, setSeconds] = useState(0);
//   const timerRef = useRef<NodeJS.Timeout | null>(null);
...
//   // ============= SYNC TIMER FROM TASKBOARD =============
  
//   // Read timer status from localStorage (TaskBoard saves here)
//   const getTimerStatus = () => {
//     try {
//       const stored = localStorage.getItem('taskBoardTasks');
//       if (stored) {
//         const tasks = JSON.parse(stored);
//         // Find any task with timerRunning === true
//         const runningTask = tasks.find((t: any) => t.timerRunning === true);
        
//         if (runningTask) {
//           return {
//             isRunning: true,
//             seconds: runningTask.timerElapsedSeconds || 0,
...
//       }
//     } catch (error) {
//       console.error('Failed to read timer status:', error);
//     }
//     return { isRunning: false, seconds: 0, taskId: null, projectId: null, taskTitle: null };
//   };

//   // Sync timer with TaskBoard every second
//   useEffect(() => {
//     const checkTimerStatus = () => {
//       const status = getTimerStatus();
      
//       if (status.isRunning) {
//         // Timer is running in TaskBoard
//         if (!isRunning) {
//           setIsRunning(true);
//           setSeconds(status.seconds);
//         } else {
//           // Update seconds from TaskBoard
//           setSeconds(status.seconds);
//         }
//       } else {
//         // No timer running in TaskBoard
//         if (isRunning) {
//           setIsRunning(false);
//           setSeconds(0);
//         }
...
//   // Start timer - uses global tracker
//   const handleStart = () => {
//     if (!selectedProjectId) {
//       alert('Please select a project first');
//       return;
...
//   // Stop timer - uses global tracker
//   const handleStop = () => {
//     if (isRunning && seconds > 0) {
//       if (activeProject && onStopTimer) {
//         const taskToUse = selectedTask || 'Unspecified';
...
//   const handleReset = () => {
//     // Reset timer in TaskBoard by finding and stopping the running task
//     try {
//       const stored = localStorage.getItem('taskBoardTasks');
//       if (stored) {
//         const tasks = JSON.parse(stored);
//         const runningTask = tasks.find((t: any) => t.timerRunning === true);
//         if (runningTask && onStopTimer) {
//           onStopTimer(runningTask.projectId, runningTask.title || 'Unspecified', '');
//         }
//       }
//     } catch (error) {
...
const TimeTracker: React.FC<TimeTrackerProps> = ({ 
  ...
  // Timer state synced from TaskBoard via localStorage
  const [timerFromTaskBoard, setTimerFromTaskBoard] = useState({ 
    isRunning: false, 
    seconds: 0,
  ...
  useEffect(() => {
    const syncTimerWithTaskBoard = () => {
      try {
        ...
        console.error('Failed to sync timer with TaskBoard:', error);
      }
    };

    // Sync immediately
    syncTimerWithTaskBoard();
    
    // Then sync every second to keep the timer updated
    const interval = setInterval(syncTimerWithTaskBoard, 1000);
  ...
  // Stop timer - uses global tracker
  const handleStop = () => {
    if (timerFromTaskBoard.isRunning && timerFromTaskBoard.seconds > 0) {
      if (activeProject && onStopTimer) {
        const taskToUse = selectedTask || 'Unspecified';
        onStopTimer(activeProject.id, taskToUse, notes);
      }
      setNotes('');
    } else if (timerFromTaskBoard.isRunning && timerFromTaskBoard.seconds === 0) {
      if (activeProject && onStopTimer) {
        const taskToUse = selectedTask || 'Unspecified';
        onStopTimer(activeProject.id, taskToUse, notes);
      }
    } else if (!timerFromTaskBoard.isRunning) {
      alert('No timer is currently running');
    }
  };
  ...
  // Use the synced timer state for display
  const isRunning = timerFromTaskBoard.isRunning;
  const seconds = timerFromTaskBoard.seconds;
        ...
        <div className="flex flex-col lg:flex-row gap-3 md:gap-4">
          ...
          <div className="lg:w-1/3 bg-white rounded-2xl p-3 sm:p-4 md:p-5 shadow-md border border-blue-100">
            <div className="text-center">
              ...
              {isRunning && timerFromTaskBoard.taskTitle && (
                <div className="mt-2 text-xs text-green-600 font-medium">
                  <Clock className="w-3 h-3 inline mr-1" />
                  Active: {timerFromTaskBoard.taskTitle}
                </div>
              )}
            </div>
          </div>
