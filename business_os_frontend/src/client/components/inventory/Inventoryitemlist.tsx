 ...
 * useful without opening every item.
 *
 * No longer renders its own "Active Items" header or search box — both
 * were moved up to InventoryPage.tsx so "Active Items + New" sits on the
 * same row as "Manage Stock / Export / Import" (Zoho-style single row),
 * and search now comes from the shared top navbar search bar via
 * SearchContext instead of a local search box here. The searchTerm /
 * onSearchChange props are kept on the interface for now (harmless if
 * unused) in case a local override is ever needed again.
 */
export const InventoryItemList: React.FC<InventoryItemListProps> = ({
  products,
  selectedProductId,
