...
import React from 'react';
import { X, Package, Tag, Hash, Boxes, Calendar, Eye, Edit, Plus, IndianRupee, Layers } from 'lucide-react';
import { Product } from '../../types/Inventory.types';
import { getStatusBadgeClass } from '../../utils/inventory.utils';
      ...
      <div className="bg-white rounded-xl shadow-2xl p-5 max-w-md w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center gap-3 mb-3">
          ...
          <div className="flex-1 min-w-0">
            <h3 className="text-sm font-semibold text-gray-900 truncate">{product.name}</h3>
            <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
              <span className={getStatusBadgeClass ? getStatusBadgeClass(product.status) : `px-2 py-0.5 text-[10px] font-medium rounded-full ${
        ...
        <div className="bg-gray-50 rounded-lg p-3 mb-3">
          ...
          <InfoRow label="SKU" value={product.sku} icon={Hash} />
          <InfoRow label="Category" value={product.category_name || 'Uncategorized'} icon={Tag} />
          <InfoRow label="Unit" value={product.unit} icon={Package} />
          <InfoRow
            label="Created"
            value={product.createdAt ? new Date(product.createdAt).toLocaleDateString() : 'N/A'}
            icon={Calendar}
          />
          {product.description && (
            <InfoRow label="Notes" value={product.description} icon={Layers} />
          )}
        </div>
